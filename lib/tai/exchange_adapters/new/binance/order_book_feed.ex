defmodule Tai.ExchangeAdapters.New.Binance.OrderBookFeed do
  @moduledoc """
  ...
  def subscribe_to_order_books(_pid, feed_id, symbols) do
    subscriptions =
      symbols
      |> Enum.map(fn symbol ->
        with {:ok, %OrderBook{} = snapshot} <-
               Tai.ExchangeAdapters.New.Binance.OrderBookSnapshot.fetch(symbol, @price_levels) do
